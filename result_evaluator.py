...
def __populate_table(system_name:str,rows: list[TsvFileInput]) -> list[list]:
    ...
    avarage_readability_score_difference = sum([Decimal(x.predictions_readability_score) - Decimal(x.readabilityScore) for x in rows]) / Decimal(n_of_methods)
    ...
    return [[system_name, rows[0].label, n_of_methods, avarage_readability_score_difference, ("%" + percentage_number_of_test_passed.__str__() + " (" + number_of_test_passed.__str__() + "/"+ n_of_methods.__str__() + ")" )]]


tsv = sys.argv[1]
rows: list[TsvFileInput] = utils.extract_rows(tsv)
system_name = sys.argv[1].split(".")[0]
rows = [x for x in rows if x.does_test_suite_pass != ""]
